// <copyright file="CheckConstraint.cs" company="Adam Nachman">
...
namespace AdamNachman.Build.SqlExtract
{
    ...
    public class CheckConstraint : BaseConstraint
    {
        ...
        public CheckConstraint(string parentName, string name, string ddl)
        ...
        /// <param name="name">The name of the constraint</param>
        /// <param name="ddl">The ddl of the object</param>
        public CheckConstraint(string schema, string parentName, string name, string ddl)
        ...
        protected override void LoadDdl(string ddl)
        {
            this.Prefix = SystemConstants.SetQuotedIdentifiersOn + string.Format(
                                        "IF NOT EXISTS (SELECT * FROM sys.check_constraints WHERE object_id = OBJECT_ID(N'[{2}].[{1}]') AND parent_object_id = OBJECT_ID(N'[{2}].[{0}]'))\r\nBEGIN\r\n",
                                        this.ParentName,
                                        this.Name,
                                        this.Schema);
            this.Body = ddl;
