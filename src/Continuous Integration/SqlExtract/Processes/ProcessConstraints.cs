// <copyright file="ProcessConstraints.cs" company="Adam Nachman">
...
namespace AdamNachman.Build.SqlExtract
{
    ...
    /// <summary>
    /// The ProcessConstaints class
    /// </summary>
    internal class ProcessConstraints : ProcessDatabase<BaseConstraint>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the ProcessConstraints class
        /// </summary>
        /// <param name="targetDatabase">The database against which the processor will execute</param>
        internal ProcessConstraints(Database targetDatabase)
        ...
        internal ProcessConstraints(Database targetDatabase, bool appendDatabaseNameToScripts, string appendDatabaseName)
        ...
        internal override string FileName
        {
            get
            {
                return string.Concat(this.GetFilePrefix(), "Constraints.sql");
            }
        }
        ...
        {
            Utilities.Logger.LogInformation("Extracting constraints...");
            foreach (BaseConstraint constraint in this.tables.NonForeignConstraints)
            {
                ExecuteScript(string.Concat(constraint.Schema, constraint.Name));
            }

            foreach (BaseConstraint constraint in this.views.NonForeignConstraints)
            {
                ExecuteScript(string.Concat(constraint.Schema, constraint.Name));
            }

            foreach (ForeignKey fk in this.tables.ForeignKeys)
            {
                ExecuteScript(string.Concat(fk.Schema, fk.Name));
            }
